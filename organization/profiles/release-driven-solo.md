...
aod-ae-utils の current release-driven development で使用する Operating Model を定義します。
...
- この repository は GitHub Issues が無効なため、Issue-number ticket workflow をそのまま前提にしない
- durable work identity は Pull Request / Git ref / accepted project documentation で明示し、存在しない Issue を捏造しない
...
- `docs/INDEX.md` と関連 domain documentation を public API semantics の project-specific authority として維持する。
- Rust/Cargo/just の既存 build/test/check entry points を保持する。
- crates.io publish は external consequential side effect として、candidate version / package contents / authorization を current evidence で確認する。
- After Effects SDK quirks / graphics semantics は code-only inference ではなく documented evidence に従う。
- GitHub Issues は現在無効。enablement は別の repository-setting decision とする。
